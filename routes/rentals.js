...
router.post('/' , async(req , res) => {
    ...
    if(!customer) return res.status(400).send('Invalid customer. Plese provide correct ID...');

    const movie = Movie.findById(req.body.movieId);
    if(!movie) return res.status(400).send('Invalid movie. Please provide correct ID...');
